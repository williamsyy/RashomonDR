...
def train(
    ...
    model.train()
    losses = []
    for idx, ((item, neigh),(item_y,neigh_y)) in enumerate(train_loader):
        print_now = print_freq is not None and (idx + 1) % print_freq == 0
        start = time.time()

        images = torch.cat([item, neigh], dim=0)
        total_y = torch.cat([item_y, neigh_y], dim=0)
        ...
        loss,basis,neighbor,neighbor_indices = criterion(features, log_Z, log_temp, force_resample=force_resample)
        # selet the neighbors indices
        
        if task_type == "pca":
            cosine_similarity_lst = []
            for i in range(neighbor_indices.shape[1]):
                vector = basis.unsqueeze(1) - neighbor[:,i,:].unsqueeze(1)

                vector_pca = item_y.unsqueeze(1) - total_y[neighbor_indices[:,i]].unsqueeze(1)
                cosine_similarity = torch.nn.functional.cosine_similarity(vector, vector_pca, dim=2)
                # print(vector.shape,item_y.unsqueeze(1).shape,torch.tensor(y,dtype=torch.float32).to("cuda")[neighbor_indices[:,i]].unsqueeze(1).shape)
                cosine_similarity_lst.append((1-cosine_similarity)**2)
            cosine_similarity = torch.cat(cosine_similarity_lst, dim=1)
            loss += torch.mean(cosine_similarity)*label_weights
        elif task_type == "concept":
            # standardize the basis
            basis = (basis - torch.mean(basis, dim=0)) / torch.std(basis, dim=0)
            # ignore the -1 labels from item_y
            mask = item_y != -1
            loss += torch.nn.functional.mse_loss(basis[mask,0], item_y[mask])*label_weights
...
class ContrastiveEmbedding(object):
    ...
    def __init__(
        ...
        :param task_type: str Type of task. Must be "pca" or "concept" or "none"
        """
...
class ContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, features, log_Z=None, log_temp=None, force_resample=False):
        ...
        return loss,origs,neighbors[:,1:,:],neigh_inds[:,1:]
