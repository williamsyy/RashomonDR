...
# based on https://discuss.pytorch.org/t/dataloader-much-slower-than-manual-batching/27014/6
class FastTensorRegressionDataLoader:
    """
    A DataLoader-like object for a set of tensors that can be much faster than
    TensorDataset + DataLoader because dataloader grabs individual indices of
    the dataset and calls cat (slow).
    """
    def __init__(self, neighbor_mat, y, batch_size=1024, shuffle=False, data_on_gpu=False, drop_last=False, seed=0):
        """
        Initialize a FastTensorDataLoader, and the corresponding y for the neighbor_mat
    ...
    def __next__(self):
        ...
        if self.indices is not None:
            indices = self.indices[start:end]
            batch = tuple(torch.index_select(t, 0, indices) for t in self.tensors)
            y_batch = tuple(self.y[torch.index_select(t, 0, indices)] for t in self.tensors)
        else:
            batch = tuple(t[start:end] for t in self.tensors)
            y_batch = tuple(self.y[t[start:end]] for t in self.tensors)
...
class CNE(object):
    ...
    def fit_transform(self, X, y=None, init=None, graph=None):
        "Fit the model, then transform."
        self.fit(X, init=init, y=y, graph=graph)
    ...
    def fit(self, X, y=None, init=None, graph=None):
        ...
        # print("y",y.shape)
        # create data loader
        self.dataloader = FastTensorRegressionDataLoader(self.neighbor_mat,
                                                y=y,
                                               shuffle=True,
                                               batch_size=self.cne.batch_size,
